...
환율과 유가에서만 값이 변경된 이유는, Yahoo Finance를 재수집하는 시점에 따라 과거 일부 일자의 조정 종가가 소급 갱신되기 때문입니다. KOSPI·S&P500·금리는 변동이 없었던 반면 환율(42개월)과 유가(26개월)에서만 차이가 발생한 점을 근거로, 데이터 오류가 아니라 외부 소스의 정상적 갱신으로 판단했습니다. 이러한 변경 가능성 자체를 통제하기 위해 재구축 이전 데이터를 기준 데이터로 동결해 보존했습니다.
...
참고로 조건수가 61,544로 크게 나타났으나, 최대 VIF는 5.78로 다중공선성 기준(통상 10)을 밑돕니다. 이는 변수 간 강한 선형종속이 아니라 수준 변수들의 단위·스케일 차이(예: 환율 약 1,100 대 금리 약 3)에서 비롯된 수치적 불안정으로 해석합니다.
...
다만 홀드아웃 표본이 24개월로 작아, 66.7%(24개월 중 16개월)의 방향 정확도가 무작위 추측(50%)보다 통계적으로 유의하게 높다고 단정하지는 않습니다. 방향 정확도는 보조 참고 지표로만 해석하고, 핵심 평가는 MAE·RMSE와 수준 복원 비교에 둡니다.
...
$$\widehat{\mathrm{KOSPI}}_t = \mathrm{KOSPI}_{2022\text{-}12} + \sum_{s=2023\text{-}01}^{t}\widehat{\Delta\mathrm{KOSPI}}_s$$
